...
def shift_and_resize_image(img, shift_x, shift_y, nw, nh):
    w, h, _ = img.shape
    enlarged = transform.resize(img, [nw, nh])
...
def compile_frames_to_gif(frame_dir, gif_file):
    ...
    print(frames)
    images = [transform.resize(imageio.imread(f), interp='nearest', size=0.33) for f in frames]
    imageio.mimsave(gif_file, images, duration=0.2)
    return gif_file
